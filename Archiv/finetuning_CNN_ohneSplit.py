...
# Trainingsfunktion
def train_model(model, criterion, optimizer, scheduler, train_loader, num_epochs=15, patience=3):
...
if __name__ == '__main__':
    ...
    model = train_model(model, criterion, optimizer, scheduler, train_loader, num_epochs=15)

    # Evaluierung auf dem gesamten Datensatz
    accuracy = evaluate_model(model, train_loader)
    print(f'Accuracy on full dataset: {accuracy:.4f}')

    # Beispielhafte Grad-CAM Visualisierung des Modells
    model.eval()
